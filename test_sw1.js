...
async function test() {
  ...
  const sw1 = ady.detectedChannels.find(c => c.commandId === 'SW1');
  const irEntry = sw1.responseData['0'];
  ...
  const res = await fetch('http://localhost:4735/import/impulse-response-data', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ identifier: 'SW1', data: base64Data, startTime: 0, sampleRate: 48000 }) });
  ...
  const ftres = await fetch('http://localhost:4735/measurements/' + uuid + '/eq/filter-tasks', { method: 'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ manufacturer: 'Generic', model: 'Generic', matchRangeStart: 20, matchRangeEnd: 20000, maxMatchBoost: 3, maxOverallBoost: 3, individualMaxBoost: 3 }) });
