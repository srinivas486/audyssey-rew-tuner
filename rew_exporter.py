...
# -----------------------------------------------------------------------------
# Utility: clear all measurements in REW
# -----------------------------------------------------------------------------

def clear_measurements_via_api(
    host: str = REW_API_DEFAULT_HOST,
    port: int = REW_API_DEFAULT_PORT,
) -> bool:
    """DELETE all measurements from REW via API.

    Calls ``DELETE /measurements`` which removes all measurements from REW's
    measurement list, allowing a clean import of new data.
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            print(f"  [✓] Cleared all REW measurements (HTTP {resp.status})")
            return True
    except urllib.error.HTTPError as e:
        body = e.read().decode('utf-8', errors='replace')
        print(f"  [✗] Failed to clear REW measurements (HTTP {e.code}): {body[:200]}")
        return False
    except urllib.error.URLError as e:
        print(f"  [✗] REW API not available at {host}:{port}")
        return False
    except OSError as e:
        print(f"  [✗] REW API not available at {host}:{port}")
        return False
